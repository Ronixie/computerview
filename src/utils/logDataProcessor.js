// src/utils/logDataProcessor.js (修正：流量直接使用字节数进行比较，并匹配分箱名称的数值含义)
...
function getTrafficBin(traffic_bytes_sum) {
    // ⭐️ 修正点 1：移除单位转换，直接使用原始字节数 (traffic_bytes_sum) 进行比较。
    // 边界值基于“亿”字节 (10^8) 定义。
    ...
    if (traffic_bytes_sum >= 4 * 10**9) return '40-50亿'; // >= 40 亿字节 (4 GB)
    if (traffic_bytes_sum >= 3 * 10**9) return '30-40亿'; // >= 30 亿字节
    if (traffic_bytes_sum >= 2 * 10**9) return '20-30亿'; // >= 20 亿字节
    if (traffic_bytes_sum >= 1 * 10**9) return '10-20亿'; // >= 10 亿字节
    if (traffic_bytes_sum >= 1 * 10**8) return '1-10亿';  // >= 1 亿字节
...
function processLogRecords() {
    ...
    // 3.1 & 3.2 处理 Login/TCPLOG 日志 (L1 记录填充)
    ...
    // 4. Level 2: Dimensional Aggregation (保持不变)
    ...
    for (const L1Record of L1RecordsMap.values()) {
        ...
        // ⭐️ 修正点 3：调用 getTrafficBin 时传入累加后的原始字节数
    ...
    // ⭐️ 修正点 4：Traffic Bins 保持不变，因为您要求名称是绝对正确的。
